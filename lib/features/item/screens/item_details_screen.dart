  ...
  Future<void> usdTojmd(amount) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          jmdPrice = 157.65 * double.parse(amount);
          jmdFetched = true;
        });
      }
    } catch (e) {
                                                ...
                                                Row(children: [
                                                  ...
                                                  const SizedBox(
                                                      width: Dimensions
                                                          .paddingSizeExtraSmall),
                                                  Text(
                                                    "${PriceConverter.convertPrice(itemController.cartIndex != -1 ? _getItemDetailsDiscountPrice(cart: Get.find<CartController>().cartList[itemController.cartIndex]) : priceWithAddons)} ${jmdPrice!=null && jmdPrice!>0 ? "(JMD \$ ${jmdPrice?.toStringAsFixed(2)})": usdTojmd(PriceConverter.convertPrice(itemController.cartIndex != -1 ? _getItemDetailsDiscountPrice(cart: Get.find<CartController>().cartList[itemController.cartIndex]) : priceWithAddons).substring(2))}",
                                                    textDirection:
                                                        TextDirection.ltr,
